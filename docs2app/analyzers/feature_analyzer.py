...
class FeatureAnalyzer:
    ...
    async def analyze_document(
        self, text: str, document_sections: Optional[List] = None
        ...
        explicit_features = await self._extract_explicit_features(
        ...
        if self.analysis_config.enable_implicit_features:
            implicit_features = await self._extract_implicit_features(
                text, explicit_features
            )
    ...
    async def _extract_explicit_features(
        self, text: str, sections: Optional[List] = None
    ) -> List[Feature]:
        ...
        messages = [
            ...
            AIMessage(
                role="user",
                content=(
                    f"Analysiere dieses Dokument und extrahiere alle Features:\n\n{text}"
                ),
            ),
        ]
    ...
    async def _extract_implicit_features(
        self, text: str, explicit_features: List[Feature]
    ) -> List[Feature]:
...
""",
        ...
        try:
            ...
            for feature_dict in features_data.get("implicit_features", []):
                try:
                    ...
                    logger.warning(
                        f"Could not create implicit feature from {feature_dict}: {e}"
                    )
    ...
    async def _enhance_features(
        self, features: List[Feature], text: str
    ) -> List[Feature]:
...
""",
        ...
        try:
            ...
            enhanced = Feature(
                ...
                kategorie=feature.kategorie,
                abhängigkeiten=enhancement_data.get(
                    "abhängigkeiten", feature.abhängigkeiten
                ),
                ...
                technical_requirements=enhancement_data.get(
            ...
            logger.warning(
                f"Feature enhancement failed for {feature.feature_name}: {e}"
            )
            return feature
...
""",
            ...
            AIMessage(
                role="user", content=f"Analysiere dieses Projektdokument:\n\n{text}"
            ),
        ]
    ...
    def _create_analysis_summary(
        self, features: List[Feature], implicit_features: List[Feature], metadata: Dict
    ) -> Dict:
        ...
        for feature in all_features:
            ...
            if feature.estimated_complexity:
                complexities[feature.estimated_complexity] = (
                    complexities.get(feature.estimated_complexity, 0) + 1
                )
        ...
        return {
            ...
            "avg_confidence": (
                sum(f.confidence for f in all_features) / len(all_features)
                if all_features
                else 0
            ),
            "high_priority_features": len(
                [f for f in all_features if f.priorität == "hoch"]
            ),
    ...
    def _get_feature_extraction_prompt(self) -> str:
        ...
        return """
Du bist ein Experte für Software-Anforderungsanalyse. Analysiere Dokumente\nund extrahiere alle beschriebenen Features/Funktionen.
...
"""
    ...
    def _get_implicit_analysis_prompt(self) -> str:
        ...
        return """
Du bist ein Experte für Software-Architektur. Identifiziere implizite Features,\ndie für eine Anwendung erforderlich sind, aber nicht explizit erwähnt werden.
