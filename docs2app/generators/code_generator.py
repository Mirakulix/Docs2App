...
class CodeGenerator:
    ...
    async def generate_project(
        self, analysis_result: FeatureAnalysisResult, project_name: Optional[str] = None
        ...
        claude_tasks = await self._generate_claude_tasks(
            analysis_result, project_structure
        )

        # Step 3: Create implementation plan
        implementation_plan = await self._generate_implementation_plan(
            analysis_result, claude_tasks
        )

        # Step 4: Generate README
        readme_content = await self._generate_readme(
            analysis_result, project_structure, implementation_plan
        )
    ...
    async def _generate_project_structure(
        self, analysis: FeatureAnalysisResult, project_name: str
    ) -> ProjectStructure:
        """Generate project structure based on features"""
        logger.info("Generating project structure")

        # Prepare feature summary for AI
        features_summary = self._create_features_summary(
            analysis.features + analysis.implicit_features
        )
...
""",
    ...
    async def _generate_claude_tasks(
        self, analysis: FeatureAnalysisResult, structure: ProjectStructure
    ) -> List[ClaudeTask]:
        ...
        for group_name, features in task_groups.items():
            tasks = await self._create_tasks_for_feature_group(
                features, structure, group_name
            )
    ...
    async def _create_tasks_for_feature_group(
        self, features: List[Feature], structure: ProjectStructure, group_name: str
    ) -> List[ClaudeTask]:
...
""",
    ...
    async def _generate_implementation_plan(
        self, analysis: FeatureAnalysisResult, tasks: List[ClaudeTask]
    ) -> Dict:
...
""",
    ...
    async def _generate_readme(
        self, analysis: FeatureAnalysisResult, structure: ProjectStructure, plan: Dict
    ) -> str:
...
""",
    ...
    async def _write_project_to_filesystem(self, result: CodeGenerationResult):
        ...
        with open(project_dir / "claude-tasks.json", "w", encoding="utf-8") as f:
            json.dump(
                [asdict(task) for task in result.claude_tasks],
                f,
                ensure_ascii=False,
                indent=2,
            )
    ...
    def _create_features_summary(self, features: List[Feature]) -> str:
        ...
        for feature in features:
            summary_lines.append(
                f"- {feature.feature_name} ({feature.kategorie}, {feature.priorität}): {feature.beschreibung}"
            )
        return "\n".join(summary_lines)

    def _group_features_for_tasks(
        ...
        for feature in features:
            if any(
                keyword in feature.feature_name.lower()
                for keyword in ["auth", "login", "benutzer", "user"]
            ):
                ...
                keyword in feature.feature_name.lower()
                for keyword in ["daten", "database", "speicher"]
            ):
    ...
    def _create_finalization_tasks(
        self, structure: ProjectStructure, analysis: FeatureAnalysisResult
    ) -> List[ClaudeTask]:
