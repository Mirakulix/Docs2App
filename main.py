...
@click.pass_context
def analyze(ctx, pdf_files, project_name, output_format, provider):
    ...
    async def _analyze_async():
        ...
        try:
            ...
            if not health["system_ready"]:
                ...
                console.print(
                    Panel(json.dumps(health, indent=2), title="Health Check Results")
                )
                sys.exit(1)
            ...
            results = await processor.process_documents(
                list(pdf_files), project_name=project_name, output_format=output_format
            )
    ...
    results = asyncio.run(_analyze_async())
    ...
    output_file = (
        Path(config_manager.config.output.directory)
        / f"analysis_results_{project_name or 'project'}.json"
    )
...
def health_check(ctx):
    ...
    health = asyncio.run(_health_check_async())
    ...
    console.print(
        f"📋 Configuration: [{config_color}]{'✅ Valid' if config_status['valid'] else '❌ Issues'}[/{config_color}]"
    )
...
@click.option(
    "--provider",
    type=click.Choice(["ollama", "openai", "azure"]),
...
def config(ctx, provider, show):
    ...
    try:
        ...
        if show:
            ...
            # AI Provider info
            console.print(
                f"🤖 Active Provider: [cyan]{config_manager.get_active_ai_provider()}[/cyan]"
            )
...
@click.option("--project-name", "-p", help="Name for the generated project")
@click.pass_context
def batch(ctx, directory, project_name):
    ...
    analyze.callback(ctx, pdf_files, project_name, "all", None)
...
def _display_results(results: dict, output_format: str):
    ...
    console.print(
        f"📄 [green]{len(successful)}/{len(extraction_results)} PDFs processed successfully[/green]"
    )
    ...
    if "feature_analysis" in results:
        ...
        console.print(
            f"   Average Confidence: {features['analysis_summary']['avg_confidence']:.2f}"
        )
